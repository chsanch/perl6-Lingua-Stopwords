...
sub get-list ( Str $list = 'snowball' ) is export {
    ...
    given $list {
        when 'all' {
            @word-list = <  aber alle allem allen aller alles als also am an ander andere anderem anderen anderer anderes anderm andern anderr anders auch auf 
                            ...
                            weshalb wie wieder wieso will wir wird wirst wo woher wohin wollen wollte während würde würden zu zum zur zwar zwischen über >;
        }
        when 'ranks-nl' {
            @word-list = <  aber als am an auch auf aus bei bin bis bist da dadurch daher darum das daß dass dein deine dem den der des dessen deshalb die 
